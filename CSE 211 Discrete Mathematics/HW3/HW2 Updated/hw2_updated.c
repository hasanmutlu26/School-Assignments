...
/*This is the updated version of HW2. The only updated part is the transitiveCheck function, which is the last function of the code.*/
...
/*All the parts above were present in HW2.*/


/*-------------------------------*/


/* THE FOLLOWING PART IS UPDATED FROM HW2.*/
void transitiveCheck(FILE* fp2, char B[BSIZE][3], int n){
    int i, j, k, flag = 0;
    /*There are 3 for loops to check three different pairs.*/
    for(i=0;i<n;i++){
        for(j=0;j<n;j++){
            for(k=0;k<n;k++){
                /*If indexes of B arrays are equal, continue without checking transivity as they are same pair.*/
                if(i==j || j==k)
                    continue;
                /*If any of the pairs are symmetric, continue without checking transivity.*/
                if(B[i][0] == B[i][2] || B[j][0] == B[j][2] || B[k][0] == B[k][2])
                    continue;
                /*If the following condition, which is the condition of transivity is satisfied, fprint the result.*/
                else if(B[i][0] == B[k][0] && B[i][2] == B[j][0] && B[j][2] == B[k][2]){
                    fprintf(fp2,"Transitive: Yes. %c%c%c is found since %c%c%c and %c%c%c are found.\n",B[k][0],B[k][1],B[k][2],B[i][0],B[i][1],B[i][2],B[j][0],B[j][1],B[j][2]);
                    flag = 1;                  /*Turn the flag to 1.*/
                    k=n;    i=n;    j=n;    /*Stop the loops.*/
                }
            }
        }
    }
    /*If flag comes out as 0, it means that there is no transivity.*/
    if(flag == 0){
        fprintf(fp2,"Transitive: No.\n");
    }
}

/* ^ THIS PART IS UPDATED FROM HW2.*/
