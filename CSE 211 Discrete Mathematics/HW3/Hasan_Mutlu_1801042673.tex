...
\subproblem{a} \solution\\ There isn't any Hamilton circuit in this graph. Because it is not possible to cross to inner or outer circles without leaving one vertex haven't been visited. If there were only two of them, there would have been a Hamilton circuit.
\subproblem{b} \solution\\ The vertices e, g and h are connected to rest of the graph with only one edge. It is not possible to visit them without having to cross the bridge again.
\subproblem{c} \solution\\ There are a lot of Hamilton circuits in this graph. One example is:
\newline
a-$>$b-$>$c-$>$f-$>$i-$>$h-$>$g-$>$d-$>$e-$>$a
\newpage
...
\solution
\newline
\includegraphics[scale=0.9]{problem3.jpg}
\newpage
